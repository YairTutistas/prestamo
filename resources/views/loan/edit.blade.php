        ...
        <form action="{{route('updateLoan', Crypt::encryptString($loan->id))}}" method="POST" class="row g-3">
            ...
            <div class="col-md-12 mt-3">
                ...
                <select name="portafolio_id" id="portafolio" class="form-control" required>
                    @foreach ($portafolios as $portafolio)
                        <option value="{{Crypt::encryptString($portafolio->id)}}" {{$loan->portafolio_id == $portafolio->id ? 'Selected' : '' }}>{{$portafolio->name}}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-6 mt-3">
                    ...
                    <select name="client_id" id="client" class="form-control" required>
                        <option value="{{Crypt::encryptString($loan->client_id)}}">{{$loan->client->name}}</option>
                    </select>
            </div>
            <div class="col-md-6 mt-3">
                ...
                <select name="company_id" id="company" class="form-control" required>
                    @foreach ($companys as $company)
                        <option value="{{ Crypt::encryptString($company->id) }}" {{ $company->id == $loan->company_id ? 'selected' : '' }}>{{ $company->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-6 mt-3">
